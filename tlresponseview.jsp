...
background-image: url(images/abstract-luxury-gradient-blue-background-smooth-dark-blue-with-black-vignette-studio-banner_1258-52630.webp);
...
<%--   <td style="text-align: center;"><%=t6 %></td>  --%>
   <td><a href="manageres.jsp?id=<%=rs.getString(1)%>&&task=<%=rs.getString(2)%>&&filename=<%=rs.getString(4)%>&&tlmail=<%=rs.getString(5)%>&&team=<%=rs.getString(6)%>&&tlkey=<%=rs.getString(7)%>&&semail=<%=rs.getString(9)%>&&mkey=<%=t6%>"><button class="btn btn-primary" onclick="fun();">ACCEPT</button></a><br><br>
        <a href="manageDELETE.jsp?idtask=<%=rs.getString(1)%>"><button class="btn btn-danger" >Reject</button></a></td> 
